...
Think of Context Engineering as the _mise en_ place for an agent—the crucial step where a
...
is ingested. Additionally, effective reasoning often relies on in-context learning^2 (a process
...
history in managed solutions like Agent Engine Sessions^3.
...
individual histories where each agent maintains its own perspective^4. The choice between
...
events of the sub-agent would be written to the same session as the root agent^5 :
...
a standard tool, passing inputs and receiving a final, self-contained output^6. With the Agent-
to-Agent (A2A) Protocol, agents use a structured protocol for direct messaging^7.
...
between these isolated agents is Agent-to-Agent (A2A) communication^8. While this pattern
...
is never persisted using tools like Model Armor^9 , you simplify compliance with privacy
...
latency-sensitive applications^10 :
...
As a specialized, decoupled service, a “ _memory manager_ ” provides the foundation for multi-
...
_Note: some frameworks may also refer to Sessions or verbatim conversation as “short-term
memory.” For this whitepaper, memories are defined as extracted information, not the raw
dialogue of turn-by-turn conversation._
...
cognitive science^11 : **declarative memories** ( _“knowing what”_ ) and **procedural memories**
( _“knowing how”_ ).
...
typically employ one or more of the following patterns to organize memories: Collections^12 ,
...
The collections^13 pattern organizes content into multiple self-contained, natural language
...
You also need to consider _who_ or _what_ a memory describes. This has implications on what
...
personalized experience for each individual; for example, _“the User prefers the middle seat.”_
...
_“the User is shopping for tickets between New York and Paris between November 7, 2025 and
November 14, 2025. They prefer direct flights and the middle seat”_. It creates a persistent
...
application; for example, _“The codename XYZ refers to the project....”_ This scope is used
to provide shared context, broadcast system-wide information, or establish a baseline of
common knowledge. A common use case for application-level memories is _procedural
memories_ , which provide "how-to" instructions for the agent; the memories are generally
...
memory is _derived from_ (its **source** ) and the data the memory is stored as (its **content** ).
...
For example, you can generate memories from multimodal input^16 using Agent Engine
...
with Memory Bank only requires a simple API call^17 :
...
predefined JSON schema or a template using LLM features like structured output^18 ; the LLM
...
persisted by providing your own topic definitions and few-shot examples^19 :
...
prompt^20. This condensed history provides the necessary context to extract key information
...
Second, an LLM is presented with both the _existing memories_ and the _new information_. Its
...
**- CR E ATE:** If the new insight is entirely novel and unrelated to existing memories, create a
    new one.
...
The classic machine learning axiom of _"garbage in, garbage out"_ is even more critical for
LLMs, where the outcome is often _"garbage in, confident garbage out."_ For an agent to make
...
To o l^21 that the agent decides to invoke when it deems the conversation meaningful to persist.
...
Bank to be consolidated with the user's existing memories^22 :
...
produce a more accurate final list^24.
...
The final architectural decision for retrieval is _when_ to retrieve memories. One approach is
...
PreloadMemoryTool or a custom callback^25 :
...
with _"Memory-as-a-Tool”_ given that the system prompt must be finalized before the LLM
...
high-level structure^26 :

**1. Extraction:** Procedural extraction requires specialized prompts designed to distill a
    reusable _strategy_ or "playbook" from a successful interaction, rather than just capturing a
...
poisoning^28. In the same way, the system must validate and sanitize information before
...
Model Armor to validate and sanitize information before committing it to long-term memory^29.
...
content _within_ the Session's history or a single request payload. Furthermore, security is
...
on _facts_ ) to make the agent an expert on the _user_. Memory is an active, LLM-driven ETL
...
2 7. https://cloud.google.com/blog/products/ai-machine-learning/rlhf-on-google-cloud
