...
Building on the previous Agents whitepaper^1 and Agent Companion^2 ; this guide provides
...
```
Chain-of-Thought^4 or ReAct^5 ) to break down complex goals into steps and decide when
...
headache. A large language model's capacity to do _anything_ makes it difficult to compel it to
do _one specific thing_ reliably and perfectly. What we used to call “prompt engineering” and
...
But how does this system actually _work_? What does an agent do from the moment it receives
...
**4. Take Action:** The orchestration layer executes the first concrete step of the plan.
    ...
    or querying a database. This is the agent _acting_ on the world beyond its own
    internal reasoning.
**5. Observe and Iterate:** The agent observes the _outcome_ of its action. The get_
...
by the **Model** , and executed by the **To o l s** until the agent's internal plan is complete and the
...
_"The user wants a delivery status. To provide a complete answer, I need a multi-step plan:_

**_1. Identify:_** _I must first find the order in our internal database to confirm it exists and get_
    _its details._
**_2. Track:_** _From those details, I'll need to extract the shipping carrier's tracking number. Then,_
    _I'll query the external carrier's API for the live status._
**_3. Report:_** _Finally, I must synthesize the information I've gathered into a clear, helpful_
    _response for the user."_
...
For an architect or product leader, a key initial decision is scoping _what kind_ of agent to build.
...
it would be unable to answer. That game is a specific, real-world event that happened _after_
...
We know what an agent does and how it can scale. But how do we actually _build_ it?
...
intersection of quality, speed, and price for _your_ specific task^8.
...
coded but is a key strategy for optimizing both performance and cost^9.

The same principle applies to handling diverse data types. While a natively multimodal
model like Gemini live mode^10 offers a streamlined path to processing images and audio,
an alternative is to use specialized tools like the Cloud Vision API^11 or Speech-to-Text API^12.
...
practice^13. With a robust CI/CD pipeline that continuously evaluates new models against your
...
transforming it from a knowledgeable assistant into an autonomous actor^14.
...
connections, and orchestration^15. Longstanding standards like the **OpenAPI** specification
...
popular because they are more convenient^16. Additionally, a few models have native tools,
...
support runtime and everything else in one platform^21. For software developers who want to
...
deployed onto industry standard runtimes like Cloud Run or GKE^22.
...
automated testing for your agents^23.
...
Agents are not tools^26. Let’s assume you already have tools wired into your agents, now let’s
...
API^31 enable bidirectional streaming, allowing a user to speak to an agent and interrupt it, just
...
by techniques like prompt injection^33. Instead, the best practice is a hybrid, defense-in-
...
the agent's proposed plan _before_ it's executed, flagging potentially risky or policy-violating
...
_principal_ that requires its own verifiable identity. Just as employees are issued an ID badge,
...
standards like SPIFFE^35 ), it can be granted its own specific, least-privilege permissions. The
...
through code and configuration^37.
...
security policies into concrete, reliable code^38.
...
URLs^40. By offloading these complex security tasks to a dedicated service, developers can
...
platform must offer dedicated, guaranteed capacity, such as Provisioned Throughput^41 for
LM services or 99.9% Service Level Agreements (SLAs) for runtimes like Cloud Run^42. This
...
**Model** (the "Brain"), the actionable **To o l s** (the "Hands"), and the governing **Orchestration
