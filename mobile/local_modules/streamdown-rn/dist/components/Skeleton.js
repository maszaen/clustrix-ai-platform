 ...
 */
export const Skeleton = ({ width = '100%', height = 16, borderRadius = 4, style, }) => {
 ...
 */
export const SkeletonText = ({ width = '100%', lines = 1, lineHeight = 14, gap = 8, style, }) => {
    if (lines === 1) {
    ...
    return (<View style={[styles.textContainer, style]}>
      {Array.from({ length: lines }).map((_, index) => {
            ...
            return (<Skeleton key={index} width={lineWidth} height={lineHeight} borderRadius={4} style={index < lines - 1 ? { marginBottom: gap } : undefined}/>);
        })}
    </View>);
};
/**
 ...
 */
export const SkeletonNumber = ({ width = 40, style, }) => {
