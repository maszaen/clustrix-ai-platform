 ...
 */
function closeUnclosedStrings(json) {
 ...
 */
export function tryParseIncompleteJSON(json) {
 ...
 */
export function extractChildrenRecursive(children) {
    return children
        .filter((child) => typeof child === 'object' && child !== null && 'c' in child)
        .map(child => ({
        name: child.c,
 ...
 */
function findBalancedClose(content, openChar, closeChar) {
    let depth = 0;
 ...
 * Props are sanitized to prevent XSS via malicious URLs.
 */
function extractSingleComponentData(content) {
    let props = {};
    let style;
    // Look for p:{...}
 ...
 */
function extractPartialChildren(childrenContent) {
    const children = [];
 ...
 * Props are sanitized to prevent XSS via malicious URLs.
 */
export function extractComponentData(content) {
    ...
    if (fullMatch) {
        ...
        const parsed = tryParseIncompleteJSON(innerJson);
        if (parsed) {
            props = parsed.p ?? {};
            style = parsed.style;
