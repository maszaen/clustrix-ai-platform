...
export function detectBlockType(line) {
    ...
    if (headingMatch) {
        return {
            type: 'heading',
            meta: { type: 'heading', level: headingMatch[1].length },
        };
    }
 ...
 */
export function detectPartialBlockType(content) {
    ...
    if (PARTIAL_BLOCK_PATTERNS.heading.test(firstLine)) {
        const level = firstLine.length;
        return {
            type: 'heading',
            meta: { type: 'heading', level: Math.min(level, 6) },
            confidence: 'likely',
        };
    }
    // Code block: ``` or ~~~
...
//# sourceMappingURL=blockPatterns.js.map