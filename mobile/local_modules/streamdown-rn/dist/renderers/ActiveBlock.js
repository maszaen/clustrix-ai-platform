 ...
 */
export const ActiveBlock = ({ block, tagState, theme, componentRegistry, }) => {
    ...
    if (block.type === 'component') {
        ...
        return (<ComponentBlock componentName={name} props={props} isStreaming={true} theme={theme} componentRegistry={componentRegistry}/>);
    }
    ...
    // Render from AST
    if (ast) {
        return (<ASTRenderer node={ast} theme={theme} componentRegistry={componentRegistry} isStreaming={true}/>);
    }
    // Fallback if parsing fails (shouldn't happen)
    return null;
};
ActiveBlock.displayName = 'ActiveBlock';
//# sourceMappingURL=ActiveBlock.js.map