...
export const StreamdownRN = React.memo(({ children, componentRegistry, theme = 'dark', style, onError, onDebug, isComplete = false, }) => {
    ...
    // When isComplete=true, uses fast batch processing instead of char-by-char
    const registry = useMemo(() => {
        ...
        try {
            // Check if content is completely new (not an append)
            // This happens when switching between messages or recycling list items
            const previousContent = previousContentRef.current;
            
            // Content is "new" if:
            // 1. Previous was not empty AND new doesn't start with previous (different content entirely)
            // 2. OR previous is empty but we have content AND isComplete (first render of saved message)
            // This ensures batch processing is triggered on initial load
            const isNewContent = (previousContent.length > 0 && !children.startsWith(previousContent))
                || (previousContent.length === 0 && children.length > 0 && isComplete);
            
            // Reset registry for completely new content to enable batch processing
            if (isNewContent) {
                registryRef.current = INITIAL_REGISTRY;
            }
            
            // Process from where we left off
            // Pass isComplete to enable fast batch processing for saved messages
            let updated = processNewContent(registryRef.current, children, isComplete);
            ...
            previousContentRef.current = children; // Track for next render
            return updated;
        }
        catch (error) {
    ...
    useEffect(() => {
        ...
        // Emit snapshot
        onDebug(snapshot);
        // Update time ref for next iteration (content ref is updated in useMemo now)
