 ...
 */
export function escapeSetextUnderlines(markdown) {
    ...
    return lines.map((line, index) => {
        ...
        if (isUnindentedListMarker(indent, chars))
            return line;
        // Preserve horizontal rules (3+ unindented dashes after blank line)
        if (isHorizontalRule(indent, chars, prevLine))
            return line;
...
/**
 * Parse a single block of markdown and return the parsed AST.
 * Returns the root node containing all children to prevent content loss.
 *
 * @param content - Block content to parse
 * @returns MDAST node (root with children, or single block), or null if empty/invalid
 */
export function parseBlockContent(content) {
    ...
    try {
        ...
        // If only one child, return it directly for backwards compatibility
        // If multiple children, return root to preserve all content
        if (ast.children.length === 1) {
            return ast.children[0];
        } else if (ast.children.length > 1) {
            // Return root node to keep all children - prevents content loss
            return ast;
        }
        return null;
    }
