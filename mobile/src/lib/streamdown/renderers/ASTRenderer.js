...
function renderNode(node, theme, componentRegistry, isStreaming = false, key) {
    ...
    switch (node.type) {
        // ========================================================================
        // Root node (multiple blocks from malformed markdown)
        // ========================================================================
        case 'root':
            return (<>{renderChildren(node, theme, componentRegistry, isStreaming)}</>);
...
function renderList(node, theme, componentRegistry, isStreaming = false, key) {
    ...
    // Use start from AST (remark parses this from markdown like "45. Item")
    const startNum = node.start ?? 1;
    
    return (<View key={key} style={{ marginBottom: theme.spacing.block, paddingLeft: 10, }}>
      {node.children.map((item, index) => {
          const itemNum = startNum + index;
          // Dynamic width based on number of digits
          const numWidth = ordered ? (itemNum < 10 ? 15 : itemNum < 100 ? 25 : itemNum < 1000 ? 33 : 40) : 14;
          return (
            <View key={index} style={{ flexDirection: 'row', marginBottom: 4 }}>
              <Text style={[styles.body, { width: numWidth }]}>
                {ordered ? `${itemNum}.` : '◦'}
              </Text>
              <View style={{ flex: 1 }}>
