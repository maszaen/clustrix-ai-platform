...
function setupEventListeners() {
  ...
  // ===== APP VERSION MODAL HANDLERS =====
  const openAppVersionBtn = document.getElementById('open-app-version');
  const closeAppVersionBtn = document.getElementById('close-app-version-modal');
  ...
  const downloadSpeedInfo = document.getElementById('download-speed-info');
  const releaseNotesSection = document.getElementById('release-notes-section');
  ...
  async function loadCurrentVersion() {
    try {
      ...
      if (result.success) {
        currentVersionSpan.textContent = `v${result.version}`;
      }
    } catch (e) {
      log("APP_VERSION", 3, "loadCurrentVersion", "Failed to load version", { error: e.message });
    }
  }

  // Open modal
  if (openAppVersionBtn) {
    openAppVersionBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      openModalWithAnimation(appVersionModal);
      closeDropdownWithAnimation($("#settings-menu"));
      ...
      log("UI", 0, "event:open-app-version", "App Version modal opened");
    });
  }

  // Close modal
  if (closeAppVersionBtn) {
    closeAppVersionBtn.addEventListener('click', () => {
      closeModalWithAnimation(appVersionModal);
      log("UI", 0, "event:close-app-version-modal", "App Version modal closed");
    });
  }

  // Close on overlay click
  if (appVersionModal) {
    appVersionModal.addEventListener('click', (e) => {
      if (e.target.classList.contains('modal-overlay')) {
        closeModalWithAnimation(appVersionModal);
      }
    });
  ...
  if (checkUpdateBtn) {
    checkUpdateBtn.addEventListener('click', async () => {
      if (updateState === 'idle' || updateState === 'checked') {
        ...
        updateStatusMessage.textContent = 'Checking for updates...';

        log("APP_VERSION", 1, "checkForUpdates", "Checking for updates");
        ...
        log("APP_VERSION", 1, "downloadUpdate", "Starting update download");
        ...
        try {
          ...
          log("APP_VERSION", 3, "downloadUpdate", "Error starting download", { error: e.message });
          updateStatusMessage.textContent = `Error: ${e.message}`;
          downloadProgressSection.classList.add('hidden');
          updateBtnText.textContent = 'Retry Download';
          checkUpdateBtn.disabled = false;
          updateState = 'available';
        }
      } else if (updateState === 'downloaded') {
        // Install update
        log("APP_VERSION", 1, "installUpdate", "Installing update");
        ...
        try {
          ...
          log("APP_VERSION", 3, "installUpdate", "Error installing update", { error: e.message });
          updateStatusMessage.textContent = `Error: ${e.message}`;
          updateBtnText.textContent = 'Retry Install';
          checkUpdateBtn.disabled = false;
        }
      }
    });
  ...
  api.on('updater:checking-for-update', () => {
    log("APP_VERSION", 1, "updater:checking-for-update", "Checking for updates");
  });

  api.on('updater:update-available', (info) => {
    log("APP_VERSION", 1, "updater:update-available", "Update available", { version: info.version });
    ...
    latestVersionSpan.textContent = `v${info.version}`;

    // Update button
    updateBtnText.textContent = 'Download Update';
    checkUpdateBtn.disabled = false;

    // Update status message
    updateStatusMessage.textContent = `A new version is available! Click the button below to download.`;
  ...
  api.on('updater:update-not-available', (info) => {
    log("APP_VERSION", 1, "updater:update-not-available", "No updates available");

    updateState = 'checked';
    updateBtnText.textContent = 'Check Again';
    checkUpdateBtn.disabled = false;
    updateStatusMessage.textContent = `You're running the latest version!`;
    updateStatusMessage.style.color = 'var(--accent-color)';

    // Reset color after a moment
    setTimeout(() => {
      updateStatusMessage.style.color = 'var(--text-secondary)';
    }, 3000);
  });

  api.on('updater:error', (error) => {
    log("APP_VERSION", 3, "updater:error", "Update error", { error: error.message });
  ...
  api.on('updater:download-progress', (progress) => {
    const percent = Math.round(progress.percent);
    downloadProgressBar.style.width = `${percent}%`;
    downloadPercentage.textContent = `${percent}%`;

    // Format speed and transferred
    ...
    downloadSpeedInfo.textContent = `${transferredMB} MB / ${totalMB} MB (${speedMBps} MB/s)`;
  });

  api.on('updater:update-downloaded', (info) => {
    log("APP_VERSION", 1, "updater:update-downloaded", "Update downloaded", { version: info.version });

    updateState = 'downloaded';
    updateBtnText.textContent = 'Install and Restart';
    checkUpdateBtn.disabled = false;
    updateStatusMessage.textContent = `Update downloaded successfully! Click the button below to install and restart.`;
    downloadProgressSection.classList.add('hidden');
  });
